...
import { Plus, Search, Pencil, Trash2, FolderKanban, Clock, User } from 'lucide-react'
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar projetos..." className="w-full pl-9 pr-4 py-2.5 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2" />
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white hover:opacity-90" style={{ backgroundColor: pc }}>
            <Plus size={16} />
            <span>Novo</span>
          </button>
        </div>
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title={editing ? 'Editar Projeto' : 'Novo Projeto'} size="md">
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={set('description')} rows={3} placeholder="Detalhes do projeto..." className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 resize-none" />
          </div>
          ...
          <div>
            ...
            <input value={form.responsible} onChange={set('responsible')} placeholder="Nome do responsável" className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2" />
          </div>
      ...
      <Modal isOpen={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Excluir Projeto" size="sm">
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { deleteProject(confirmDelete.id); setConfirmDelete(null) }} className="flex-1 py-2.5 text-sm bg-red-500 text-white rounded-xl hover:bg-red-600 font-medium">Excluir</button>
          </div>
        </div>
      </Modal>
    </div>
